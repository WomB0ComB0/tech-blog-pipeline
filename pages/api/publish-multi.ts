import { PublishRequest, type PublishRequestType } from "@lib/schemas";
import { Schema } from "effect";
import type { NextApiRequest, NextApiResponse } from "next";

interface PublishResultInternal {
	platform: string;
	success: boolean;
	article?: unknown;
	error?: string;
...
async function publishToDevTo(data: PublishRequestType): Promise<unknown> {
...
async function publishToHashnode(data: PublishRequestType): Promise<unknown> {
...
export default async function publishMulti(
	req: NextApiRequest,
	res: NextApiResponse,
) {
	if (req.method !== "POST") {
	...
	try {
		...
		const parseResult = Schema.decodeUnknownEither(PublishRequest)(req.body);
		...
		const publishData = parseResult.right;

		// Publish to each platform in parallel
		const promises = publishData.platforms!.map(
			async (platform): Promise<PublishResultInternal> => {
				try {
					let article: unknown;
					if (platform === "devto") {
