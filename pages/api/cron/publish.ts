 ...
 */
async function selectUniqueIdea() {
  ...
  const usedIdeas = allIdeas
    .filter(idea => idea.used && idea.usedAt)
  ...
  const recentEmbeddings = await Promise.all(
    usedIdeas.map(idea => generateIdeaEmbedding(idea.title, idea.description))
  );
  ...
  for (const idea of unusedIdeas) {
    const ideaEmbedding = await generateIdeaEmbedding(idea.title, idea.description);
    
    // Calculate max similarity to any recent idea
    const maxSimilarity = Math.max(
      ...recentEmbeddings.map(embedding => cosineSimilarity(ideaEmbedding, embedding))
    );
 ...
 */
async function generateArticleContent(title: string, description: string, tags: string[]) {
 ...
 */
async function publishArticle(title: string, content: string, tags: string[], platforms: string[]) {
  const response = await fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/publish-multi`, {
    method: 'POST',
    headers: {
...
export default async function cronPublish(
  ...
  try {
    ...
    const content = await generateArticleContent(idea.title, idea.description, idea.tags);
    console.log('Generated content');

    // Publish to platforms (default to both devto and hashnode)
    const platforms = (req.body?.platforms as string[]) || ['devto', 'hashnode'];
    const publishResult = await publishArticle(idea.title, content, idea.tags, platforms);
