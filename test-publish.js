...
async function testPublish(testName, data, expectedStatus = 200) {
	...
	try {
		...
		console.log(
			`Status: ${response.status} ${response.status === expectedStatus ? "✅" : "❌"}`,
		);
...
async function runTests() {
	...
	results.push(
		await testPublish("Test 1: Publish to Dev.to (draft)", {
			body: {
...
Published at: ${new Date().toISOString()}`,
	...
	results.push(
		await testPublish("Test 2: Publish to both platforms (draft)", {
			body: {
...
Published at: ${new Date().toISOString()}`,
	...
	results.push(
		await testPublish("Test 6: Publish to Medium only (draft)", {
			body: {
...
Published at: ${new Date().toISOString()}`,
	...
	console.log("\n💡 Tips:");
	console.log(
		"  - Make sure your .env.local has PUBLISH_PASSWORD, DEV_TO_API_KEY, and MEDIUM_API_KEY",
	);
